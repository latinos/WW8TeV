...
Float_t MetCuts[numberMetCuts] = {-0.9, -0.86, -0.6, 0.88, 1};
Float_t MetDraw[numberMetCuts] = {-0.9, -0.86, -0.6, 0.88, 1};
	...
	Bool_t    drawR = false)
{
  ...
  TFile* inputWgammaSF   = new TFile(path + "Wgamma.root"); 
  // Need to add Wgamma and Wgamma samples to remove!! 
  // Wgamma are 0 events in 0jet bin 
  // Wgamma are 2.47 events in 0 jet bin
  ...
  TH1F* hNinWgammaSF  [numberMetCuts];
  ...
  TH1F* hNoutWgammaSF  [numberMetCuts];
  TH1F* hNoutDataSF[numberMetCuts];
  TH1F* hNoutDataOF[numberMetCuts];

  for (Int_t nC=0; nC<numberMetCuts; nC++) {
    hNinDYSF  [nC] = (TH1F*)inputDYSF  ->Get(Form("hNinZevents%.1i",  nC+1));
    hNinWZSF  [nC] = (TH1F*)inputWZSF  ->Get(Form("hNinZevents%.1i",  nC+1));
    hNinZZSF  [nC] = (TH1F*)inputZZSF  ->Get(Form("hNinZevents%.1i",  nC+1));
    hNinWgammaSF  [nC] = (TH1F*)inputWgammaSF  ->Get(Form("hNinZevents%.1i",  nC+1));
    hNinDataSF[nC] = (TH1F*)inputDataSF->Get(Form("hNinZevents%.1i",  nC+1));
    hNinDataOF[nC] = (TH1F*)inputDataOF->Get(Form("hNinZevents%.1i",  nC+1));

    hNoutDYSF  [nC] = (TH1F*)inputDYSF  ->Get(Form("hNoutZevents%.1i", nC+1));     
    hNoutWZSF  [nC] = (TH1F*)inputWZSF  ->Get(Form("hNoutZevents%.1i", nC+1));     
    hNoutZZSF  [nC] = (TH1F*)inputZZSF  ->Get(Form("hNoutZevents%.1i", nC+1)); 
    hNoutWgammaSF  [nC] = (TH1F*)inputWgammaSF  ->Get(Form("hNoutZevents%.1i", nC+1));         
    hNoutDataSF[nC] = (TH1F*)inputDataSF->Get(Form("hNoutZevents%.1i", nC+1));
    hNoutDataOF[nC] = (TH1F*)inputDataOF->Get(Form("hNoutZevents%.1i", nC+1));
  }  
  ...
  TH1F* hNinDYmumu = (TH1F*)inputDYmumu->Get("hNinLooseZevents1");
  TH1F* hNinDYee   = (TH1F*)inputDYee  ->Get("hNinLooseZevents1");
  ...
  Double_t NinWgammaSF  [numberMetCuts];
  ...
  Double_t NoutWgammaSF  [numberMetCuts];  // Not used for now
  ...
  for (UInt_t nC=0; nC<numberMetCuts-1; nC++) {
    ...
    NinWgammaSF   [nC] = hNinWgammaSF   [nC]->GetBinContent(2);
    ...
    NoutWgammaSF  [nC] = hNoutWgammaSF  [nC]->GetBinContent(2);
  ...
  Double_t RMC     [numberMetCuts];
  Double_t errRMC  [numberMetCuts];
  ...
  for (UInt_t nC=0; nC<numberMetCuts-1; nC++) {
    ...
    if (printLevel > 2) {
      printf("\n %0.02f < dymva < %.02f ", MetCuts[nC], MetCuts[nC+1]);
      printf(" -------------------------------------------------\n");
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    /// --->> Choose as default the R from data and not from MC. 
    R   [nC] = RData   [nC];
    errR[nC] = errRData[nC]; 
  ...
  Double_t NinCountedSFWgamma   = NinWgammaSF  [sysR];
  ...
  if (printLevel > 0) {
    ...
    printf(" \\hline \n"); 
    printf(" same flavour  &  %5.3f $\\pm$ %5.3f  &              %4.0f  &  %5.1f $\\pm$ %4.1f $\\pm$ %4.1f  &  %5.1f $\\pm$ %4.1f  &     %4.1f \\\\  \n",
	 R[theR],
	   errR[theR],
	   NinCountedSFData,
	   yield,
  ...
  if (drawR) {
    ...
    for (UInt_t i=0; i<numberMetCuts-1; i++) {
      ...
      gR->SetPoint     (i, 0.5 * (MetDraw[i+1] + MetDraw[i]),    RMC[i]);
      gR->SetPointError(i, 0.5 * (MetDraw[i+1] - MetDraw[i]), errRMC[i]);

      gRdata->SetPoint     (i, 0.5 * (MetDraw[i+1] + MetDraw[i]),    RData[i]);
      gRdata->SetPointError(i, 0.5 * (MetDraw[i+1] - MetDraw[i]), errRData[i]);

      if (absoluteMin > (RMC[i]     - errRMC[i]))     absoluteMin = RMC[i]     - errRMC[i];
